...
@interface SLCheckFlightVC ()<UITableViewDelegate,UITableViewDataSource,SLCheckFlightInfoViewDelegate>
...
@implementation SLCheckFlightVC{
    
    BOOL _timeSort;
    BOOL _priceSort;
    
    SLCheckFlightInfoView *_mFilterView;
}
...
- (void)viewDidLoad {
    ...
    if (self.mBackFlightInfoDIC) {
        //往返
        self.mLB_fromCity.text = self.mBackFlightInfoDIC[@"toCityStr"];
        self.mLB_arrivalCity.text = self.mBackFlightInfoDIC[@"fromCityStr"];
        self.mLB_time.text = [NSString stringWithFormat:@"%@  %@",[[self.mBackFlightInfoDIC[@"backDate"] substringFromIndex:5] stringByReplacingOccurrencesOfString:@"-" withString:@"月"],[self getWeekDayWithStr:self.mBackFlightInfoDIC[@"fromDate"]]];
        [self.mBackFlightInfoDIC setObject:self.mBackFlightInfoDIC[@"backDate"] forKey:@"fromDate"];
        ...
        self.mLB_time.text = [NSString stringWithFormat:@"%@  %@",[[self.mFlightInfoDIC[@"fromDate"] substringFromIndex:5] stringByReplacingOccurrencesOfString:@"-" withString:@"月"],[self getWeekDayWithStr:self.mFlightInfoDIC[@"fromDate"]]];
...
-(void)getFightInfoWithNetWorking:(void (^)(bool result))comple{
    ...
    if (self.mBackFlightInfoDIC) {
        ...
        NSString *formCitystr = self.mBackFlightInfoDIC[@"toCity"];
        NSString *tomCitystr = self.mBackFlightInfoDIC[@"fromCity"];
        
        [dic setObject:formCitystr forKey:@"fromCity"];
        [dic setObject:tomCitystr forKey:@"toCity"];
        
    }else{
        dic = self.mFlightInfoDIC;
    }
    
    [HttpApi searchFlightList:dic SuccessBlock:^(id responseBody) {
        // LDLOG(@"%@",responseBody);
        
        NSArray *mFights = responseBody[@"flights"];
        NSArray *mAirports = responseBody[@"airports"];
        NSArray *mAirLines = responseBody[@"airlines"];
        self.mAirports = mAirports;
        self.mAirlines = mAirLines;
        
        NSError *error;
        NSArray *temp = [MTLJSONAdapter modelsOfClass:[SLCheckFightModel class] fromJSONArray:mFights error:&error];
        
        self.mFights = temp;
        
        for (SLCheckFightModel *tempModel in temp) {
        ...
        for (int i = 0; i<self.mDataSoure.count; i++) {
            if (tempModel) {
                SLCheckFightModel *tempModel2 = temp[i];
        ...
        self.mLB_numofAllFlight.text = [NSString stringWithFormat:@"共有%lu个航班信息",(unsigned long)[self.mDataSoure count]];
...
- (IBAction)onclickYesterdayBtn:(UIButton *)sender {
    ...
    NSDate *date;
    if (self.mBackFlightInfoDIC) {
        date  =[formatter dateFromString:self.mFlightInfoDIC[@"backDate"]];
...
- (IBAction)onclickOneDayAfterBtn:(UIButton *)sender {
    ...
    if (self.mBackFlightInfoDIC) {
        [self.mBackFlightInfoDIC setObject:destDateString forKey:@"backDate"];
          self.mLB_time.text = [NSString stringWithFormat:@"%@  %@",[self.mBackFlightInfoDIC[@"backDate"] substringFromIndex:5],[self getWeekDayWithStr:self.mFlightInfoDIC[@"backDate"]]];
    }else{
    ...
    [self getFightInfoWithNetWorking:^(bool result) {
        ...
        if (self.mBackFlightInfoDIC) {
            self.mLB_time.text = [NSString stringWithFormat:@"%@  %@",[self.mBackFlightInfoDIC[@"backDate"] substringFromIndex:5],[self getWeekDayWithStr:self.mBackFlightInfoDIC[@"backDate"]]];
...
- (IBAction)onclickFilterBtn:(UIButton *)sender {
    ...
    if (_mFilterView == nil) {
        _mFilterView = [[NSBundle mainBundle]loadNibNamed:@"SLCheckFlightInfoView" owner:nil options:nil][0];
    }
...
- (IBAction)onclickPriceFilerBtn:(UIButton *)sender {
    ...
    NSSortDescriptor *descriptor = [NSSortDescriptor sortDescriptorWithKey:@"_mPrice" ascending:!_priceSort];
...
- (IBAction)onclickTimeFilerBtn:(UIButton *)sender {
    ...
    NSSortDescriptor *descriptor = [NSSortDescriptor sortDescriptorWithKey:@"_mFormTime" ascending:_timeSort];
    NSArray *descriptors = [NSArray arrayWithObject:descriptor];
    [self.mDataSoure sortUsingDescriptors:descriptors];
    [self.mInfoTableView reloadData];
}
- (IBAction)onclickLowPirceBtn:(UIButton *)sender {
    NSSortDescriptor *descriptor = [NSSortDescriptor sortDescriptorWithKey:@"_mPrice" ascending:YES];
    NSArray *descriptors = [NSArray arrayWithObject:descriptor];
    [self.mDataSoure sortUsingDescriptors:descriptors];
    [self.mInfoTableView reloadData];
}
...
-(void)SLCheckFlightInfoView:(SLCheckFlightInfoView *)view onclickHideFightBtn:(UIButton *)sender{
    ...
    if ([sender.titleLabel.text isEqualToString:@"显示共享航班"]) {
        [sender setTitle:@"隐藏共享航班" forState:UIControlStateNormal];
        
        [self.mDataSoure removeAllObjects];
        [self.mDataSoure addObjectsFromArray:self.mFights];
    }else{
        [sender setTitle:@"显示共享航班" forState:UIControlStateNormal];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([SLSimpleInterest shareNetWork].TicketType == SLFlightTrackVC_TicketTrip_Air ) {
        cellIndentifier = @"SLCheckFlightCell";
    }else{
        cellIndentifier = @"SLCheckTrainCell";
    }
    
    if(MainScreenFrame_Height == 667.0){
        if ([SLSimpleInterest shareNetWork].TicketType == SLFlightTrackVC_TicketTrip_Air ) {
            cellIndentifier = @"SLCheckFlightCellPUS";
        }else{
            cellIndentifier = @"SLCheckTrainCellPUS";
        }
    ...
    [tableView registerNib:[UINib nibWithNibName:cellIndentifier bundle:[NSBundle mainBundle]] forCellReuseIdentifier:cellIndentifier];
    SLCheckFlightCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIndentifier];
