...
- (void)updateDay:(NSArray*)number outDate:(NSArray*)outdateArray selected:(NSInteger)judge currentDate:(NSArray*)newArray;
{
    ...
    //设置日期
    NSString *timeStr = [number componentsJoinedByString:@""];
    
//    NSMutableString *mTempTimeStr = [NSMutableString stringWithFormat:@"%@",timeStr];
//    if ([mTempTimeStr containsString:@"-"]) {
//       timeStr =  [mTempTimeStr stringByReplacingOccurrencesOfString:@"-" withString:@""];
//    }
//    NSDate *mDate = [MyFounctions convertDateFromString:timeStr];
//    NSCalendar * myCalendar = [NSCalendar currentCalendar];
//    myCalendar.timeZone = [NSTimeZone systemTimeZone];
//    NSInteger week = [[myCalendar components:NSWeekdayCalendarUnit fromDate:mDate] weekday];
//    
//    if(week == 6){
//      self.number.textColor =[UIColor redColor];
//    }

    NSString *mCurrectStr = [MyFounctions getCurrentDateWithDateFormatter:@"yyyyMMdd"];;
    if([timeStr isEqualToString: mCurrectStr]){
        
        self.number.text = @"今天";
        [self.number setTextColor:SL_BULE];
